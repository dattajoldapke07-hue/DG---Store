...
function filterCat(id, name) { 
    ...
    const filtered = id==='all' ? allProducts : allProducts.filter(p => p.cat === id);
    renderProducts(filtered); 
}
...
function renderProducts(arr) {
    document.getElementById("products").innerHTML = arr.map(p => {
        ...
        return `
        <div class="product">
            <i class="fa fa-heart wish-icon ${isWished ? 'active' : ''}" onclick="toggleWishlist(${p.id})"></i>
            <img src="${p.img}">
            <div class="p-info">
                <div style="font-size:0.7rem; color:#f59e0b; margin-bottom:5px;">
                    <i class="fa fa-star"></i> ${rating} | <span style="color:var(--accent)">Only ${stock} Left</span>
                </div>
...
function toggleWishlist(id) {
    ...
    if(idx > -1) wishlist.splice(idx, 1); else wishlist.push(p);
    updateUI();
    renderProducts(allProducts); 
}
...
function updateUI() {
    ...
    document.getElementById("cartItems").innerHTML = cart.map(c => {
        ...
        return `<div style="display:flex; gap:10px; margin-bottom:10px;">
            <img src="${c.img}" style="width:40px; height:40px; object-fit:cover; border-radius:5px;">
            <div style="flex:1; font-size:0.8rem;"><b>${c.name}</b><br>₹${c.price} x ${c.qty}</div>
        </div>`;
    }).join('');
    document.getElementById("totalPrice").innerText = t;

    document.getElementById("wishlistItems").innerHTML = wishlist.map(w => `
        <div style="display:flex; gap:10px; margin-bottom:10px; border-bottom:1px solid #f1f5f9; padding-bottom:5px;">
            <img src="${w.img}" style="width:40px; height:40px; object-fit:cover;">
            <div style="flex:1; font-size:0.8rem;"><b>${w.name}</b><br>₹${w.price}</div>
            <button onclick="addToCart(${w.id})" style="border:none; background:var(--dark); color:white; padding:5px; border-radius:5px;">Add</button>
        </div>`).join('');
}
...
function setPay(m) { 
    ...
    if(m === 'upi') document.getElementById("upiBox").classList.remove("hidden");
    else if(m === 'qr') document.getElementById("qrBox").classList.remove("hidden");
}

function processOrder() {
    const ph = document.getElementById("cPhone").value;
    const addr = document.getElementById("cAddr").value;
    if(ph.length < 10 || !addr || !selectedPay) return alert("Fill all details!");

    document.getElementById("checkoutForm").classList.add("hidden");
    document.getElementById("finalBill").classList.remove("hidden");
    
    document.getElementById("resName").innerText = sessionStorage.getItem("user");
    document.getElementById("resPhone").innerText = ph;
    document.getElementById("resDate").innerText = new Date().toLocaleString();
    
    let payName = selectedPay === 'qr' ? 'Scan & Pay (Datta Madhavrao Jold)' : selectedPay.toUpperCase();
    document.getElementById("resPay").innerText = payName;

    const orderId = Math.floor(Math.random() * 900000) + 100000;
    document.getElementById("bId").innerText = orderId;

    let h = ''; let total = 0;
    cart.forEach(c => {
        total += (c.price * c.qty);
        h += `<div style="display:flex; justify-content:space-between; font-size:0.8rem; margin-bottom:5px;"><span>${c.name} x${c.qty}</span><b>₹${c.price*c.qty}</b></div>`;
    });
    document.getElementById("bItems").innerHTML = h;
